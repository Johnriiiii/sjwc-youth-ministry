...
adminRoutes.patch('/submissions/:id/status', async (req, res) => {
    ...
    const updated = await SubmissionModel.findByIdAndUpdate(id, { status }, { new: true }).lean();
    if (!updated) {
...
adminRoutes.patch('/submissions/:id', async (req, res) => {
    ...
    if (typeof body.middleName === 'string')
        patch.middleName = body.middleName.trim();
    ...
    if (typeof body.registrationDate === 'string')
        patch.registrationDate = body.registrationDate;
    if (typeof body.contactNumber === 'string')
        patch.contactNumber = body.contactNumber.trim();
    if (typeof body.address === 'string')
        patch.address = body.address.trim();
    if (typeof body.guardianContact === 'string')
        patch.guardianContact = body.guardianContact.trim();
...
adminRoutes.post('/activities', async (req, res) => {
    const { title, details, eventDate, location, status } = req.body;
...
adminRoutes.patch('/activities/:id', async (req, res) => {
    ...
    if (typeof body.details === 'string')
        patch.details = body.details.trim();
    if (typeof body.eventDate === 'string')
        patch.eventDate = body.eventDate;
